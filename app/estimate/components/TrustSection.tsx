...
export function TrustSection() {
  return (
    <section className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      <div className="mb-6">
        <p className="text-xs font-black uppercase tracking-[0.18em] text-[#314874]">Trust</p>
        <h2 className="mt-2 text-3xl font-black text-[#1E2E4F]">ทำไมต้องประเมินราคากับ SP Kansard?</h2>
      </div>
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {cards.map(({ title, description, Icon }) => (
          <article key={title} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm transition hover:-translate-y-0.5 hover:shadow-lg">
            <div className="grid h-12 w-12 place-items-center rounded-2xl bg-[#eaf4ff] text-[#314874]">
              <Icon />
            </div>
